...
2. Controlleer of nginx active and running is
   ...
   Loaded: loaded (/usr/lib/systemd/system/nginx.service; enabled; vendor prese>
    Drop-In: /usr/lib/systemd/system/nginx.service.d
           └─php-fpm.conf
   Active: active (running) since Thu 2020-11-26 10:00:01 UTC; 15min ago
    Process: 15759 ExecReload=/bin/kill -s HUP $MAINPID (code=exited, status=0/SU>
    Process: 15790 ExecStart=/usr/sbin/nginx (code=exited, status=0/SUCCESS)
    Process: 15789 ExecStartPre=/usr/sbin/nginx -t (code=exited, status=0/SUCCESS)
    Process: 15786 ExecStartPre=/usr/bin/rm -f /run/nginx.pid (code=exited, statu>
    Main PID: 15792 (nginx)
    Tasks: 2 (limit: 5025)
   Memory: 2.3M
   CGroup: /system.slice/nginx.service
           ├─15792 nginx: master process /usr/sbin/nginx
           └─15793 nginx: worker process
...
**Komt overeen**
   ...
   ```
   **zoals testrapport2 komt overeen**
   ```
...
**Ik moet systemctl status postgresql intypen en bekom een iets ander resultaat, postgreSQL vind ik niet**

   ```
   ● postgresql.service - PostgreSQL database server
   Loaded: loaded (/usr/lib/systemd/system/postgresql.service; enabled; vendor >
   Active: active (running) since Thu 2020-11-26 10:00:04 UTC; 18min ago
  Process: 15849 ExecStartPre=/usr/libexec/postgresql-check-db-dir postgresql (>
 Main PID: 15852 (postmaster)
    Tasks: 8 (limit: 5025)
   Memory: 34.1M
   CGroup: /system.slice/postgresql.service
           ├─15852 /usr/bin/postmaster -D /var/lib/pgsql/data
           ├─15854 postgres: logger process
           ├─15856 postgres: checkpointer process
           ├─15857 postgres: writer process
           ├─15858 postgres: wal writer process
           ├─15859 postgres: autovacuum launcher process
           ├─15860 postgres: stats collector process
           └─15861 postgres: bgworker: logical replication launcher
...
*Extra databases/gebruiker kunnen bestaan als er uitbreiding zijn gedaan op de basis config
 ...
 2. Log in op PSQL account
   ...
   ```
 3. Controleer of de database drupal bestaat
...
**Zoals in testrapport 2 heb ik hier een extra user**

 4. Controleer of de gebruiker drupal is aangemaakt
      Commando:
...
-----------+------------------------------------------------------------+-----------
 ...
 rainloop  |                                                            | {}
   ```
**Opnieuw zoals in testrapport 2 heb ik 1 extra rij**
...
**Opnieuw zoals in testrapport2 heb ik 2 rijen minder**
...
**Heb de bravo server opgestart, deze is up and running. Ik heb zelf uitgezocht hoe ik mijn primary dns aanpas (aangezien dit nergens in het document staat uitgelegd) maar ik kan de pagina nog steeds niet laden en krijg volgende fout DNS_PROBE_FINISHED_NXDOMAIN**

**Aanpassing primary dns: control pannel > Network and Internet > Network and Sharing Centre > change adapter settings > Ethernet > Properties > Internet Protocol Version 4 (TCP/IPv4)  > Properties > Preferred DNS server**

**gelieve alles eens te bekijken, alle fouten aan te passen en indien de verschillen normaal en verwacht zijn, zet het er dan in**